...
export default async function ScopesLinesPage({ params, searchParams }: { params: Promise<{ id: string }>; searchParams: Promise<{ error?: string; q?: string; collapsed?: string }> }) {
  ...
  <div className="space-y-3"><form action={addScopeAction.bind(null, id)} className="flex flex-col gap-2 sm:flex-row sm:items-end"><div className="grid gap-2"><Label htmlFor="scope_id">{dictionary.scopesLines.addScope}</Label><Select name="scope_id" disabled={availableScopes.length === 0}><SelectTrigger id="scope_id" className="w-full sm:w-[220px]"><SelectValue placeholder={dictionary.scopesLines.selectScope} /></SelectTrigger><SelectContent>{availableScopes.map((scope) => <SelectItem key={scope.id} value={scope.id}>{scope.name}</SelectItem>)}</SelectContent></Select></div><Button size="sm" disabled={availableScopes.length === 0}>{dictionary.scopesLines.addScopeButton}</Button></form><PageActionBar primary={{ key: "calculate", href: `/offers/${id}/calculation`, label: dictionary.pageActions.calculate.label, ariaLabel: dictionary.pageActions.calculate.aria }} secondary={[{ key: "edit-prices", href: `/offers/${id}/material-prices`, label: dictionary.pageActions.editPrices.label, ariaLabel: dictionary.pageActions.editPrices.aria }]} tertiary={[{ key: "back-overview", href: `/offers/${id}/overview`, label: dictionary.pageActions.backToOverview.label, ariaLabel: dictionary.pageActions.backToOverview.aria }]} /></div></CardContent></Card>
